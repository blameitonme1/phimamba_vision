...
for epoch in range(1):
    for (images, labels) in tqdm(dataloader):
        ...
        student_outputs = student_model(images).last_hidden_state  # logits -> log probabilities
        student_outputs = nn.functional.log_softmax(student_outputs, dim=1)  # log probabilities
        # change dtype
        # print(student_outputs.shape, teacher_outputs.shape)
        # loss_kl = criterion_kl(student_outputs, teacher_outputs)
        loss_ce = criterion_ce(student_outputs, teacher_outputs)
        loss = loss_ce
